...
<head>
    <title>
        @if($order->status !== \App\Enums\OrderStatus::CONFIRMED)
            Yêu cầu hủy đơn hàng của bạn đã được phê duyệt
        @else
            Yêu cầu hủy đơn hàng của bạn không được phê duyệt
        @endif
    </title>
    <meta charset="UTF-8">
</head>
...
@if($order->status !== \App\Enums\OrderStatus::CONFIRMED)
    <p>Chúng tôi xin thông báo rằng yêu cầu hủy đơn hàng cho đơn hàng #{{ GenerateOrderNumber($order->id) }} của quý khách đã được phê duyệt.</p>
@else
    <p>Chúng tôi xin thông báo rằng yêu cầu hủy đơn hàng cho đơn hàng #{{ GenerateOrderNumber($order->id) }} của quý khách không được phê duyệt.</p>
@endif

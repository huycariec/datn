...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $currentMonthRevenue = Order::where('status', 'received')
            ->whereMonth('created_at', Carbon::now()->month)
            ->sum('total_amount');

        $lastMonthRevenue = Order::where('status', 'received')
        ...
        $rawRevenue = Order::where('status', 'received')
        ...
        $topCategories = Category::select('categories.id', 'categories.name', DB::raw('COALESCE(SUM(orders.total_amount),0) as revenue'))
            ...
            ->where(function($q) {
                $q->whereNull('orders.status')
                    ->orWhere('orders.status', 'received');
            })
        ...
        $topProducts = Product::select(
            'products.name',
            DB::raw('SUM(order_details.quantity) as total_quantity_selled'),
            DB::raw('SUM(order_details.unit_price * order_details.quantity) as revenue_product')
        )
        ->join('order_details', 'products.id', '=', 'order_details.product_id')
        ->join('orders', 'order_details.order_id', '=', 'orders.id')
        ->where('orders.status', 'received')
        ->groupBy('products.id', 'products.name')
        ->orderByDesc('total_quantity_selled')
        ->limit(5)
        ->get()
        ->map(function($item) {
            return (object)[
                'name' => $item->name,
                'total_quantity_selled' => (int)$item->total_quantity_selled,
                'revenue_product' => (float)$item->revenue_product,
            ];
        });
